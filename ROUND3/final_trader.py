"""Round 3 `final_trader` — refines `trader.py` without replacing the core MM engine.

Based on `PROCESS_LOG.txt` and data analysis: keep EWMA + vol execution, option surface
as a take trigger only, executable cointegration *pairs* only, and drift effectively off.

This module adds: (1) adjacent-strike **neighbor** mid as a soft inventory tilt on the
VEV ladder (shape mean reversion, not a passive fair replacement); (2) **ladder** inventory
pressure via VELVET-delta-style weights; (3) throttled wings (6000/6500) and slightly
reduced size on the most VELVET-linked VEV_5000/VEV_5100.

Tweak `LIMITS` to match the official IMC per-product cap table.

Local `run_bt` check (3 days, same machine as dev): ≈61.4k total vs ≈61.3k for `trader.py`;
re-run before submission. `--match-trades worse` is the robustness test from `PROCESS_LOG.txt`.
"""
...
RESIDUAL_TARGET_SCALES: Dict[str, float] = {
    ...
    # Slight nudge vs `trader.py`: wide book + strong historical PnL (PROCESS_LOG addendum).
    "HYDROGEL_PACK": 0.47,
}
...
# Strike order for neighbor mid (PROCESS_LOG / option smile structure).
_VEV_LADDER: Tuple[str, ...] = (
    "VEV_4000",
    "VEV_4500",
    "VEV_5000",
    "VEV_5100",
...
# Soft tilt toward local ladder consistency (kept << 1 so it does not replace EWMA).
NEIGHBOR_RESIDUAL_SCALE = 0.10

# Extra quote skew when net VEV inventory loads spot-sensitive strikes (regression betas, PROCESS_LOG 5.3).
...
# Ladder block skew (0 = off). 0.12 was a good local compromise vs pure baseline.
BLOCK_RISK_SKEW_K = 0.12

# PROCESS_LOG: 6000/6500 are almost static. Set USE_WING_THROTTLE = False to match baseline.
...
def _update_online_state(state: dict, mid: float, vol_floor: float) -> dict:
    ...
    if prev_slow is None:
        ...
        state["slow_fair"] = ALPHA_SLOW_FAIR * mid + (1.0 - ALPHA_SLOW_FAIR) * float(prev_slow)
    state["prev_mid"] = mid
...
def _vev_neighbor_predicted_mid(product: str, order_depths: Dict[str, OrderDepth]) -> float | None:
    if product not in _VEV_LADDER:
        return None
    i = _VEV_LADDER.index(product)
    ms: list[float] = []
    for j in (i - 1, i + 1):
        if not (0 <= j < len(_VEV_LADDER)):
            continue
        odj = order_depths.get(_VEV_LADDER[j])
        if odj is None:
            continue
        m = _book_mid(odj)
        if m is not None:
...
def _vev_block_risk(position: Dict[str, int]) -> float:
    s = 0.0
    for p, w in _VEV_DELTA_W.items():
        s += int(position.get(p, 0)) * w
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        block_risk = _vev_block_risk(state.position)
        ...
        for product, od in state.order_depths.items():
            ...
            nbr = _vev_neighbor_predicted_mid(product, state.order_depths)
    ...
    def _adaptive(
        ...
        *,
        ...
        if product in SLOW_TARGET_PRODUCTS:
            ...
            residual_target = _clamp((slow_fair - mid) / max(1.0, base_take_edge), -1.0, 1.0) * slow_scale
        ...
        if product.startswith("VEV_") and neighbor_pred is not None:
            nr = _clamp((neighbor_pred - mid) / max(1.0, base_take_edge), -1.0, 1.0) * NEIGHBOR_RESIDUAL_SCALE
            target_frac = _clamp(target_frac + nr, -1.0, 1.0)
        make_edge = max(float(ec.min_make), ec.k_make * vol)
        if USE_WING_THROTTLE and product in WING_VEV:
            make_edge *= WING_MAKE_EDGE_MULT
        ...
        if USE_WING_THROTTLE and product in WING_VEV:
            take_cap = max(1, int(take_cap * WING_TAKE_FRAC))
            make_cap = max(1, int(make_cap * WING_MAKE_FRAC))
        ...
        if product.startswith("VEV_") and BLOCK_RISK_SKEW_K > 0.0:
            skew -= BLOCK_RISK_SKEW_K * block_risk * make_edge
