...
class Trader:
    """
    IMC Prosperity 4 Round 5 v11 structural profit-lock.
    ...
    Changes versus the 28.86k-PnL v4 bot:
    ...
    - Keeps v4 product priors and execution unchanged.
    - Adds v10 surgical leak-cut: disables the three remaining v9 negatives without changing winner mechanics.
    ...
    # Structural v11 improvement:
    # v10 has almost no final negative products, but multiple profitable products give
    # back large mid-run peaks. These rules do not change the core fill mechanics.
    # They only stop opening new risk and flatten inventory after product-level PnL
    # has made a meaningful peak and then decayed by a product-specific trailing amount.
    # tuple = (minimum_peak_pnl, trailing_giveback, earliest_timestamp)
    PROFIT_LOCK_RULES = {
        "ROBOT_MOPPING": (1800.0, 420.0, 90000),
        "SNACKPACK_STRAWBERRY": (1200.0, 520.0, 62000),
        "GALAXY_SOUNDS_SOLAR_FLAMES": (1200.0, 500.0, 78000),
        "SLEEP_POD_POLYESTER": (1500.0, 500.0, 60000),
        "SNACKPACK_VANILLA": (800.0, 360.0, 68000),
        "SLEEP_POD_SUEDE": (900.0, 400.0, 96000),
        "UV_VISOR_MAGENTA": (900.0, 380.0, 65000),
        "ROBOT_VACUUMING": (650.0, 330.0, 74000),
        "GALAXY_SOUNDS_SOLAR_WINDS": (900.0, 300.0, 97000),
    }
    ...
    def _save_memory(self, mem: Dict[str, Any]) -> str:
        ...
        mem["seen_own_trades"] = mem.get("seen_own_trades", [])[-self.MAX_SEEN_TRADES:]
    ...
    def _update_product_pnl_locks(self, mem: Dict[str, Any], state: TradingState, fairs: Dict[str, float]) -> None:
        cash = mem.setdefault("cash", {})
        product_pnl = mem.setdefault("product_pnl", {})
        pnl_peak = mem.setdefault("pnl_peak", {})
        locked = mem.setdefault("profit_locked", {})
        ...
        for product, trades in getattr(state, "own_trades", {}).items():
            for tr in trades:
                ...
                # Cash convention: buying spends cash, selling receives cash.
                if buyer == "SUBMISSION":
                    cash[product] = float(cash.get(product, 0.0)) - price * qty
                if seller == "SUBMISSION":
                    cash[product] = float(cash.get(product, 0.0)) + price * qty

        mem["seen_own_trades"] = seen_list[-self.MAX_SEEN_TRADES:]
        ...
        for product, fair in fairs.items():
            ...
            pnl_peak[product] = peak

            if locked.get(product, False):
                continue

            rule = self.PROFIT_LOCK_RULES.get(product)
            if rule is None:
                continue

            min_peak, trail, earliest = rule
            if state.timestamp >= earliest and peak >= min_peak and peak - pnl >= trail:
                locked[product] = True

        mem["total_pnl"] = total
        mem["total_peak"] = max(float(mem.get("total_peak", total)), total)

    def _is_profit_locked(self, mem: Dict[str, Any], product: str) -> bool:
        return bool(mem.get("profit_locked", {}).get(product, False))
    ...
    def run(self, state: TradingState):
        ...
        self._update_product_pnl_locks(mem, state, fairs)
        ...
        for product, depth in state.order_depths.items():
            ...
            if product in self.AVOID or self._is_profit_locked(mem, product):
