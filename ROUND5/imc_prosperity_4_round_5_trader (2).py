...
class Trader:
    """
    IMC Prosperity 4 Round 5 final research-backed bot.

    Architecture:
    - Passive-first market making because uploaded data showed residual signals were usually
      not large enough to pay the spread.
    - Product selection because repeated winners/losers dominated the round.
    - Product-specific alpha modes: mean reversion, trend, or family residual.
    - Counterparty/trader-ID learning as a capped overlay, because Round 5 public meta says
      revealed trader IDs can matter, but uploaded logs did not include enough complete
      counterparty history to make it the only edge.
    - Strict hard limit 10 and conservative directional caps.
    """
    ...
    # Hard avoid list from repeated losses in both uploaded runs.
    AVOID = {
    ...
    # Names that looked profitable mainly through spread capture / skewed passive quoting.
    PASSIVE_ONLY = {
    ...
    # Product-specific modes from uploaded run comparisons + family/return diagnostics.
    MODE = {
        "MICROCHIP_SQUARE": "mr",
    ...
    def _save(self, mem: Dict[str, Any]) -> str:
        for bucket, keep in (("fair", self.HIST), ("resid", self.RESID_HIST)):
            for p in list(mem.get(bucket, {}).keys()):
                mem[bucket][p] = mem[bucket][p][-keep:]
        mem["pending_alpha"] = mem.get("pending_alpha", [])[-1200:]
        mem["cp_pending"] = mem.get("cp_pending", [])[-2000:]
        mem["seen_trades"] = mem.get("seen_trades", [])[-4000:]
        if len(mem.get("cp_score", {})) > 1000:
            items = sorted(
                mem["cp_score"].items(),
                key=lambda kv: abs(float(kv[1])),
                reverse=True,
            )
            mem["cp_score"] = dict(items[:700])
    ...
    def _fair(self, depth: OrderDepth) -> Optional[float]:
    ...
    def _resolve_alpha_markouts(self, mem: Dict[str, Any], timestamp: int, fairs: Dict[str, float]) -> None:
        ...
        for ev in mem.get("pending_alpha", []):
            p = ev.get("p")
            if p not in fairs or timestamp - int(ev.get("t", timestamp)) < self.MARKOUT_DELAY:
                keep.append(ev)
                continue
            move = fairs[p] - float(ev.get("fair", fairs[p]))
            score = self._clip((move * float(ev.get("sign", 0.0))) / max(1.0, float(ev.get("vol", 1.0))), -4.0, 4.0)
            rec = mem["perf"].get(p, {"s": 0.0, "n": 0})
            n = int(rec.get("n", 0))
            lr = 0.08 if n < 10 else 0.04
            rec["s"] = (1.0 - lr) * float(rec.get("s", 0.0)) + lr * score
            rec["n"] = min(9999, n + 1)
            mem["perf"][p] = rec
    ...
    def _resolve_counterparty_markouts(self, mem: Dict[str, Any], timestamp: int, fairs: Dict[str, float]) -> None:
        ...
        for ev in mem.get("cp_pending", []):
            ...
            score = self._clip((move * float(ev.get("sign", 0.0))) / max(1.0, float(ev.get("vol", 1.0))), -4.0, 4.0)
            k = ev.get("k", "")
            old = float(mem["cp_score"].get(k, 0.0))
            mem["cp_score"][k] = 0.96 * old + 0.04 * score
        mem["cp_pending"] = keep
    ...
    def _trade_key(self, product: str, tr: Any) -> str:
        return f"{product}|{getattr(tr, 'timestamp', '')}|{getattr(tr, 'buyer', '')}|{getattr(tr, 'seller', '')}|{getattr(tr, 'price', '')}|{getattr(tr, 'quantity', '')}"
    ...
    def _counterparty_flow(self, state: TradingState, mem: Dict[str, Any], product: str, fair: float, vol: float) -> float:
        ...
        for tr in state.market_trades.get(product, []):
            ...
            if buyer and buyer != "SUBMISSION":
                ...
                kf = f"F|{fam}|{buyer}"
                score = 0.7 * float(mem["cp_score"].get(kp, 0.0)) + 0.3 * float(mem["cp_score"].get(kf, 0.0))
                flow += w * score
                if not already_seen:
        ...
        mem["seen_trades"] = seen_list[-4000:]
    ...
    def _alpha(self, mode: str, mr: float, trend: float, family: float, micro: float, cp: float) -> float:
        if mode == "trend":
    ...
    def _add(self, orders: List[Order], product: str, price: int, qty: int, pos: int, buy_used: int, sell_used: int) -> Tuple[int, int]:
        if qty > 0:
    ...
    def _flatten_orders(self, product: str, depth: OrderDepth, pos: int) -> List[Order]:
        orders: List[Order] = []
        bid, _, ask, _ = self._best(depth)
        if bid is None or ask is None or pos == 0:
            return orders
        buy_used = 0
        sell_used = 0
        if pos > 0:
            buy_used, sell_used = self._add(orders, product, bid, -min(pos, 3), pos, buy_used, sell_used)
        elif pos < 0:
            buy_used, sell_used = self._add(orders, product, ask, min(-pos, 3), pos, buy_used, sell_used)
        return orders
    ...
    def _make_orders(self, product: str, depth: OrderDepth, fair: float, alpha: float, target: int, pos: int, vol: float, timestamp: int) -> List[Order]:
        ...
        if timestamp >= self.FLATTEN_START:
            return self._flatten_orders(product, depth, pos)

        spread = max(1.0, ask - bid)
        buy_used = 0
        sell_used = 0

        # Selective taker leg. Only active products can cross, and only for small size.
    ...
    def run(self, state: TradingState):
        ...
        fam_mu = {fam: self._mean(vals) for fam, vals in fam_logs.items() if vals}
        ...
        for product, depth in state.order_depths.items():
            ...
            if product in self.AVOID:
                result[product] = self._flatten_orders(product, depth, pos)
                continue
            ...
            alpha = self._alpha(mode, mr, trend, family, micro, cp)
