...
class Trader:
    """
    IMC Prosperity 4 Round 5 v32 isolated microchip book-flow lift.

    This version returns to the strict v17 core and applies the v30 book-flow/queue-drain
    microstructure overlay to exactly one product: MICROCHIP_SQUARE.

    Why:
    - v31 did not change behavior because its stateless OBI-only lift almost never fired.
    - v30 showed MICROCHIP_SQUARE could gain strongly from book-flow lift, but the basket
      overlay damaged fragile core products.
    - v32 keeps the v30 signal that mattered, but isolates it to the single product where
      the experiment showed the cleanest upside.
    - No exact timestamp exits, no peak locks, no hard product shutdowns, no branch overrides,
      and no quote-side reducers.
    """
    ...
    # v32: isolate the only microstructure overlay that showed enough upside in v30.
    # All other products remain exact v17 behavior because broad overlays hurt the core.
    MICRO_LIFT = {
        "MICROCHIP_SQUARE",
    }
    ...
    def _save_memory(self, mem: Dict[str, Any]) -> str:
        ...
        # Keep microstructure state compact. These are generic per-product statistics,
        # not large event histories.
        for bucket in ("book_ema", "book_prev"):
            if len(mem.get(bucket, {})) > 80:
                mem[bucket] = dict(list(mem[bucket].items())[-80:])
    ...
    def _multi_level_imbalance(self, depth: OrderDepth) -> float:
        ...
        v17 already used top-of-book imbalance. This adds depth-2/depth-3 information
        without turning it into a hard gate. It is intentionally small and generic.
        Positive = buy pressure / ask side likely to be consumed.
        Negative = sell pressure / bid side likely to be consumed.
        """
        ...
        for i in range(min(len(bids), len(asks), 3)):
            ...
            if den <= 0:
                continue
            total += weights[i] * ((bv - av) / den)
            wsum += weights[i]

        if wsum <= 0:
            return 0.0
        return self._clip(total / wsum, -1.0, 1.0)

    def _book_flow_signal(self, mem: Dict[str, Any], product: str, depth: OrderDepth) -> float:
        """
        Queue-drain proxy, not a fake deterministic queue tracker.

        In the official runtime we cannot observe our exact queue position across ticks,
        and submitted orders should be treated as one-tick decisions. What we can observe
        robustly is whether the visible bid/ask queues are being depleted or repriced.

        Positive = ask queue is draining / prices improving upward.
        Negative = bid queue is draining / prices repricing downward.
        """
        bid, bid_v, ask, ask_v = self._best(depth)
        if bid is None or ask is None:
            return 0.0

        prev = mem.setdefault("book_prev", {}).get(product)
        raw = 0.0

        if isinstance(prev, dict):
            pb = prev.get("b")
            pa = prev.get("a")
            pbv = int(prev.get("bv", 0))
            pav = int(prev.get("av", 0))

            # Price movement is stronger evidence than quantity movement.
            if pb is not None:
                if bid > pb:
                    raw += 0.55
                elif bid < pb:
                    ...
                    drain = (pbv - bid_v) / max(1.0, float(pbv))
                    if drain > 0.35:
                        raw -= min(0.45, 0.45 * drain)
            ...
            if pa is not None:
                if ask > pa:
                    ...
                    drain = (pav - ask_v) / max(1.0, float(pav))
                    if drain > 0.35:
                        raw += min(0.45, 0.45 * drain)

        top = self._imbalance(depth)
        multi = self._multi_level_imbalance(depth)
        raw = self._clip(raw + 0.28 * top + 0.22 * multi, -1.5, 1.5)

        ema_map = mem.setdefault("book_ema", {})
        old = float(ema_map.get(product, 0.0))
        ema = 0.72 * old + 0.28 * raw
        ema_map[product] = self._clip(ema, -1.5, 1.5)

        mem["book_prev"][product] = {"b": bid, "bv": bid_v, "a": ask, "av": ask_v}
        return self._clip(ema_map[product], -1.25, 1.25)
    ...
    def _signals(
        ...
        # v32: only the selected microstructure-lift basket receives multi-level OBI.
        # All other products use exact v17 top-book imbalance to preserve the passive core.
        if product in self.MICRO_LIFT:
            micro = self._clip(0.62 * self._imbalance(depth) + 0.38 * self._multi_level_imbalance(depth), -1.0, 1.0)
        else:
    ...
    def _combine_alpha(
        ...
        if product in self.TREND_NAMES:
            alpha = 0.55 * rel + 0.25 * own_mr + 0.75 * trend + 0.25 * micro + 0.45 * flow + 0.32 * tape + 0.18 * book_flow + 0.25 * inv
        elif product in self.FAMILY_NAMES:
            alpha = 1.20 * rel + 0.20 * own_mr + 0.18 * trend + 0.30 * micro + 0.45 * flow + 0.22 * tape + 0.16 * book_flow + 0.25 * inv
        else:
            alpha = 1.10 * rel + 0.35 * own_mr + 0.10 * trend + 0.32 * micro + 0.45 * flow + 0.25 * tape + 0.16 * book_flow + 0.28 * inv
        ...
        # v32: if tape and queue-drain proxy both disagree with weak RV, soften conviction.
        # This is deliberately weaker than v26/v27 quote cuts: it changes target strength,
        # not product participation.
        if abs(book_flow) > 0.85 and abs(tape) > 0.55 and (book_flow * tape) > 0 and (rv * book_flow) < 0 and abs(rv) < 1.15:
            alpha *= 0.86
    ...
    def _make_orders(
        ...
        # v32: queue/OBI pressure slightly shifts reservation in the direction of
        # current book flow. This is a placement skew, not a shutdown gate.
        reservation = fair + (0.30 * alpha + 0.10 * book_flow) * max(1.0, spread) - inv_frac * max(1.0, 0.75 * spread + 0.15 * vol)
        ...
        # v32: additive queue-pressure lift. When alpha and queue-drain agree,
        # improve quote placement by one tick inside a wide spread. This should
        # increase good fills rather than reduce participation.
        if spread >= 4 and product in self.MICRO_LIFT and alpha * book_flow > 0.75 and abs(alpha) > 0.80:
            if alpha > 0:
        ...
        # v32: additive participation boost. Earlier attempts failed by cutting
        # fills. This only adds size when alpha, tape/book pressure, and inventory
        # room agree; it never zeros a side.
        if product in self.MICRO_LIFT and alpha * book_flow > 0.65 and abs(alpha) > 0.70:
        ...
        if product in self.MICRO_LIFT and abs(tape) > 0.75 and tape * alpha > 0.20 and product not in self.CAUTIOUS:
            if alpha > 0 and position < cap - 1:
                buy_sz = max(buy_sz, 1)
            elif alpha < 0 and position > -cap + 1:
    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            # v32: book-flow is allowed to influence only the selective lift basket.
            # For protected/core products this is exactly v17 behavior with book_flow=0.
            book_flow = self._book_flow_signal(mem, product, depth) if product in self.MICRO_LIFT else 0.0
            ...
            alpha = self._combine_alpha(product, rel, own_mr, trend, micro, flow, tape, book_flow, position, scale)

            target = self._target_position(product, position, alpha, scale, vol, spread)
            result[product] = self._make_orders(product, depth, fair, alpha, target, position, vol, spread, scale, tape, book_flow)
