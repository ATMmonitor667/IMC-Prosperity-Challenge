...
class Trader:
    """
    IMC Prosperity 4 Round 5 v34 deeper persistent-trend escape.
    ...
    v31 changes after the 576215/v30 run:
    - Remove the adaptive expert-router; it repeated the v26-v29 participation-collapse failure.
    - Raise only live-positive participation, never from static timestamps or one-run exits.
    - Preserve one passive two-sided quote on strong products when not near cap, because the
      best run earned most of its edge from spread capture plus inventory skew.

    v33 changes after the 576972/v32 run:
    - Revert v32's proof-of-edge cap/floor restrictions because they reduced total PnL.
    - Keep v31's cap-9 strong-product participation guard intact.
    - Add a generic persistent-trend escape: when a non-trend product is in a strong sustained
      trend and the relative-value signal is not decisive, stop blindly fading the move.
      This is not timestamp-specific and does not hard-code a product exit.

    v34 changes after the 577151/v33 run:
    - Keep the 31.4k v33 base completely intact.
    - Strengthen trend escape only when the trend is unusually persistent and RV/MR evidence
      is not strong enough to fight it.
    - This remains a state-based rule: no timestamp exits, no product locks, no run-specific stops.
    """
    ...
    def _combine_alpha(
        ...
        # v33: generic persistent-trend escape.
        #
        # v31 broke 30k by preserving passive participation. v32 lost because it changed
        # the participation engine too much. The clean remaining problem is not fill count;
        # it is what happens when a product that is normally treated as mean-reverting starts
        # making a sustained directional move. In that case, fading relative value can become
        # stale. This rule is deliberately broad: no timestamps, no peak locks, no hard-coded
        # product shutdowns. It only shifts a fraction of alpha toward trend when:
        #   1) the product is not already a trend-specialist,
        #   2) trend is strong,
        #   3) RV/MR disagrees with that trend,
        #   4) RV/MR is not strong enough to justify fighting the move.
        rv = rel + 0.45 * own_mr
        trend_escape = False
        if product not in self.TREND_NAMES and rv * trend < 0 and abs(trend) > 1.55 and abs(rv) < 2.05:
            trend_alpha = 0.95 * trend + 0.24 * tape + 0.18 * micro + 0.18 * inv
            alpha = 0.72 * alpha + 0.28 * trend_alpha
            trend_escape = True

        # v34: deeper trend escape for unusually persistent moves.
        # The 577151/v33 run improved because it stopped fading a strong continuation move,
        # but the residual position still did not fully respect the directional regime.
        # This stronger blend only activates when the continuation signal is extreme and
        # the RV/MR package is not decisive. It is intentionally generic and state-based:
        # no product exits, no timestamp exits, and no fitted PnL locks.
        deep_escape = False
        if product not in self.TREND_NAMES and rv * trend < 0 and abs(trend) > 2.15 and abs(rv) < 2.35:
            trend_alpha = 1.15 * trend + 0.28 * tape + 0.22 * micro + 0.16 * inv
            alpha = 0.58 * alpha + 0.42 * trend_alpha
            trend_escape = True
            deep_escape = True

        # If trend strongly disagrees with mean-reversion/relative-value, reduce conviction.
        # During a v33/v34 trend-escape condition, reduce much less because the alpha has
        # already been re-centered toward the persistent move.
        if product not in self.TREND_NAMES and rv * trend < 0 and abs(trend) > 1.1:
            alpha *= 0.93 if deep_escape else (0.86 if trend_escape else 0.60)
    ...
    def _target_position(self, product: str, position: int, alpha: float, scale: float, vol: float, spread: float) -> int:
        ...
        if scale > 1.25 and abs(alpha) > 0.85:
            desired *= 1.10
    ...
    def _make_orders(
        ...
        if product in self.STRONG and spread >= 3 and abs(position) <= max(1, cap - 3) and abs(alpha) < 1.35:
            buy_sz = max(buy_sz, 1)
