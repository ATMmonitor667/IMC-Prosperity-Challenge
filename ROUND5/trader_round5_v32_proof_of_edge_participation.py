...
class Trader:
    """
    IMC Prosperity 4 Round 5 v32 proof-of-edge participation guard.
    ...
    v32 changes after the 576562/v31 run:
    - Keep v31's ceiling-breaking participation, but require live proof before using the
      extra strong-product inventory slot.
    - Strong names default to the v17 cap of 8; they earn cap 9 only after delayed markouts
      lift their live scale. This is a general online rule, not a product/timestamp hack.
    - The strong-product passive floor now backs off when live evidence has weakened, so
      bad strong-name inventory does not silently accumulate while good names still scale.
    """
    ...
    def _product_cap(self, product: str, scale: float) -> int:
        if scale <= 0:
            return 0
        if product in self.STRONG:
            # v31's blanket cap-9 was the ceiling breaker, but the 576562 run showed
            # the extra slot should be earned by live evidence.  A fresh strong product
            # starts at the v17 cap of 8; once delayed markouts turn positive the scale
            # rises above this threshold and the product receives the ninth slot.
            # This keeps the upside in proven names while reducing weak-name inventory.
            return 9 if scale >= 1.26 else 8
        if product in self.CAUTIOUS:
            return 5
        return 4
    ...
    def _target_position(self, product: str, position: int, alpha: float, scale: float, vol: float, spread: float) -> int:
        ...
        # If live evidence has weakened, do not let stale inventory expand just because
        # the static prior is strong.  This is intentionally a soft carry-cost: it trims
        # unsupported inventory without shutting down the passive core.
        if product in self.STRONG and scale < 1.12 and position * desired > 0 and abs(position) >= max(3, cap // 2):
            desired *= 0.82
    ...
    def _make_orders(
        ...
        # Only apply the floor when the product is neutral-to-good live.  This preserves
        # v31's fill count on working strong names, but prevents the floor from defending
        # stale inventory after live markouts have weakened.
        if product in self.STRONG and scale >= 1.12 and spread >= 3 and abs(position) <= max(1, cap - 3) and abs(alpha) < 1.35:
            buy_sz = max(buy_sz, 1)
