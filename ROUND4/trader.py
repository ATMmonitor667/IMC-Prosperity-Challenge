...
Implements the same `Trader.run(state)` contract as other rounds. Strategy:

  * Core: online EWMA fair value + Welford drift t-test + vol-scaled take/make
    (same family as `ROUND2/trader.py` adaptive leg).
  * Per-name tuning informed by the Round 3 surface / spot structure (if those
    products appear again): **HYDROGEL** and high-strike **VEV_*** have wide
    top-of-book spreads in historical data → we scale minimum edges and sizes;
    **very low** mids (wings) use tight integer edges and a vol floor to avoid
    stalling in noise.

All unknown product names go through the same adaptive logic with a default
limit and default edge scale. Tweak `LIMITS` / `run_bt.py` to match the
official per-product position caps.
"""
...
_LIMITS: Dict[str, int] = {
    ...
    "VEV_6000": 20,
    "VEV_6500": 20,
    # Add Round 4 symbols here as they are published
}
...
def _edge_config(product: str, mid: float) -> EdgeConfig:
    """Widen required edge when historical spread is large; cap sizes when wide."""
    c = _default_edge()
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        for product, od in state.order_depths.items():
            ...
            orders, product_state = self._adaptive(product, od, position, product_state)
